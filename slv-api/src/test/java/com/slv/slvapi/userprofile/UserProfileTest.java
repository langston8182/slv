...
public class UserProfileTest extends AbstractTest {
   ...
   */
  private static final String INPUT_FILE = "json/userprofile/input.json";

  /**
   * File name of UserProfile API Outputs.
   */
  private static final String OUTPUT_FILE = "json/userprofile/output.json";
  ...
  public void getProfilPropertyDescriptors() throws SlvTestsException {
    ...
    JsonDiffResult result = 
        retrieveResult(UserProfileMethods.GET_PROFIL_PROPERTY_DESCRIPTORS.getUrl());
  ...
  public void createUserProfile() throws SlvTestsException {
    ...
    Map<String, Object> inputs = 
        convert(getInputs().get(UserProfileMethods.CREATE_PROFIL.getUrl()));

    // Profil names must be equals
    String expectedProfilName = 
        (String) inputs.get(JsonAttributes.INPUT_CREATE_PROFIL_NAME.getName());
    String realProfilName = (String) responseJsonNode
        .get(JsonAttributes.OUTPUT_CREATE_PROFIL_NAME.getName());
    Assert.assertNotNull(realProfilName, MessageHelper
        .getMessage("user.profile.create.response.name.not.found"));
    Assert.assertEquals(realProfilName, expectedProfilName,
        MessageHelper.getMessage(
            "user.profile.create.response.name.not.expected", expectedProfilName, realProfilName));

    // Verify existence by calling "getGeoZoneProfils"
    response = call(UserProfileMethods.GET_GEOZONE_PROFILS.getUrl(),
        getInputs().get(UserProfileMethods.GET_GEOZONE_PROFILS.getUrl()));
    Assert.assertNotNull(response, 
        MessageHelper.getMessage("user.profile.geozoneprofils.response.null"));

    JsonNode jsonNode = convertToJsonNode(response);
    Assert.assertTrue(jsonNode.isArray(),
        MessageHelper.getMessage("user.profile.geozoneprofils.response.not.array", response));
    ...
    Assert.assertTrue(profilFound,
        MessageHelper.getMessage(
            "user.profile.create.profil.not.found.in.list", expectedProfilName));
  }

  @Test(groups = { "userProfile-createUpdateDeleteUser" },
      dependsOnMethods = { "createUserProfile" }, priority = 2)
  public void updateUserProfile() throws SlvTestsException {
    ...
    // Response must be "\"OK\""
    Assert.assertEquals(response, UPDATE_PROFIL_OK_RESPONSE,
        MessageHelper.getMessage("user.profile.update.response.not.ok", response));

    // Verify properties by calling "getGeoZoneProfils"
    response = call(UserProfileMethods.GET_GEOZONE_PROFILS.getUrl(),
        getInputs().get(UserProfileMethods.GET_GEOZONE_PROFILS.getUrl()));
    Assert.assertNotNull(response, MessageHelper
        .getMessage("user.profile.geozoneprofils.response.null"));
    ...
    Map<String, Object> inputs = convert(getInputs()
        .get(UserProfileMethods.UPDATE_PROFIL.getUrl()));
    String expectedProfilName = (String) inputs
        .get(JsonAttributes.INPUT_UPDATE_PROFIL_NAME.getName());
    ...
    Assert.assertNotNull(properties, MessageHelper
        .getMessage("user.profile.geozoneprofils.no.properties"));
    Assert.assertTrue(properties.isArray(),
        MessageHelper.getMessage("user.profile.geozoneprofils.properties.not.array", response));

    // Verify that updated properties are equals to what we sent to updateProfil
    // service
    assertUpdatedPropertiesPresence(inputs, properties);
  }

  @Test(groups = { "userProfile-createUpdateDeleteUser" },
      dependsOnMethods = { "createUserProfile" }, priority = 3)
  public void deleteUserProfile() throws SlvTestsException {
    ...
    Assert.assertTrue(responseJsonNode
        .containsKey(JsonAttributes.OUTPUT_DELETE_ERROR_CODE.getName()));
    Assert.assertEquals(responseJsonNode.get(JsonAttributes.OUTPUT_DELETE_ERROR_CODE.getName()),
        DELETE_ERROR_CODE_OK_VALUE);
    ...
    Assert.assertEquals(responseJsonNode
        .get(JsonAttributes.OUTPUT_DELETE_STATUS.getName()), DELETE_STATUS_OK_VALUE);

    // "statusOk" must be present and equals to true
    Assert.assertTrue(responseJsonNode
        .containsKey(JsonAttributes.OUTPUT_DELETE_STATUS_OK.getName()));
    Assert.assertEquals(responseJsonNode
        .get(JsonAttributes.OUTPUT_DELETE_STATUS_OK.getName()), true);

    // "statusError" must be present and equals to false
    Assert.assertTrue(responseJsonNode
        .containsKey(JsonAttributes.OUTPUT_DELETE_STATUS_ERROR.getName()));
    Assert.assertEquals(responseJsonNode
        .get(JsonAttributes.OUTPUT_DELETE_STATUS_ERROR.getName()), false);
  }

  /**
   * Verify that the properties contained in the input {@link Map} are present
   * in the property list defined by the properties parameters.
   * 
   * @param inputs
   *          a {@link Map} which contains the values sent to the updateProfil
   *          service
   * @param properties
   *          a {@link JsonNode} which represents the property list
   */
  private void assertUpdatedPropertiesPresence(Map<String, Object> inputs, JsonNode properties) {
    ...
    for (Iterator<JsonNode> iterator = properties.iterator(); iterator.hasNext();) {
      ...
      String key = property.get(JsonAttributes.PROPERTY_OBJECT_KEY.getName()) != null
          ? property.get(JsonAttributes.PROPERTY_OBJECT_KEY.getName()).asText() : null;
      // Value of the "value" attribute
      String value = property.get(JsonAttributes.PROPERTY_OBJECT_VALUE.getName()) != null
          ? property.get(JsonAttributes.PROPERTY_OBJECT_VALUE.getName()).asText() : null;

      // Search for expected values
      if (PROPERTY_VALUE_LOCALE.equals(key) && inputs
          .get(JsonAttributes.PROPERTY_LOCALE.getName()).equals(value)) {
        localeCheck = true;
      } else if (PROPERTY_VALUE_SKIN.equals(key) && inputs
          .get(JsonAttributes.PROPERTY_SKIN.getName()).equals(value)) {
        skinCheck = true;
    ...
    Assert.assertTrue(localeCheck && skinCheck && blockedActionChecked,
        MessageHelper.getMessage("user.profile.update.properties.not.updated"));
  }
